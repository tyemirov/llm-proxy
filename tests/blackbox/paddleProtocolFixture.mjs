...
const customerID='ctm_01hv8x2axb33yr5y238zfwcn5p';
...
export async function startPaddleProtocolFixture() {
  ...
  const server=http.createServer(async(request,response)=>{
    try {
      ...
      if(request.method==='GET' && url.pathname==='/customers') {
        ...
        return send([{id:customerID}]);
      }
      if(request.method==='POST' && url.pathname==='/transactions') {
        const chunks=[];for await(const chunk of request)chunks.push(chunk);
        const input=JSON.parse(Buffer.concat(chunks).toString());
        if(input.customer_id!==customerID || input.items.length!==1 || input.items[0].price_id!==priceID || input.items[0].quantity!==1 || !input.custom_data.funding_order_id || !input.custom_data.billing_account_id)throw new Error('Unbound transaction');
        const transaction={id:`txn_${String(transactions.length+1).padStart(26,'0')}`,status:'ready',currency_code:'USD',collection_mode:'automatic',customer_id:customerID,custom_data:input.custom_data,items:[{quantity:1,price:{id:priceID,unit_price:{amount:'500',currency_code:'USD'}}}]};
      ...
      if(request.method==='POST' && url.pathname===`/customers/${customerID}/portal-sessions`) {
  ...
  return {
    config:{environment:'sandbox',processor_account_id:'browser-processor',supplier_id:'browser-supplier',api_key:apiKey,api_base_url:origin,webhook_secret:webhookSecret,offers:[{code:'five',price_id:priceID,funding_cents:500}]},
    ...
    refund(orderID,status,revision) {
      ...
      const adjustment={id:'adj_00000000000000000000000001',action:'refund',type:'partial',status,transaction_id:transaction.id,customer_id:customerID,currency_code:'USD',created_at:'2026-09-23T12:01:00Z',updated_at:updated,items:[{id:'adjitm_browser',item_id:itemID,type:'partial',amount:'200',totals:{subtotal:'200',tax:'20',total:'220'}}],totals:{subtotal:'200',tax:'20',total:'220',fee:'0',retained_fee:'0',earnings:'200',currency_code:'USD'}};
