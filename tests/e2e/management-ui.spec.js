...
test("the route explorer starts with Text and exposes media capabilities through task toggles", async ({ page }) => {
  ...
  await expect(routingTree.locator('[data-task="image_generation"]')).toContainText("Generate images");
  await expect(routingTree.locator('[data-task="vision"]')).toContainText("Understand images");
