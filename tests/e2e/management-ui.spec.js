...
test("public landing explains the product and exposes the generated capability catalog", async ({ request }) => {
  ...
  expect(html).toContain('<output class="routing-tree__counts" aria-live="polite" data-route-counts>12 families · 46 exact models · 46 offerings</output>');
  ...
  expect(html).toContain('<strong>69</strong><span>Exact models</span>');
  expect(html).toContain('<strong>70</strong><span>Offerings</span>');
...
test("the routing tree and capability catalog remain complete without JavaScript", async ({ browser }) => {
  ...
  await expect(routingTree.locator("[data-route-provider]")).toHaveCount(70);
  await expect(routingTree.locator("[data-route-model]")).toHaveCount(69);
  ...
  await expect(routingTree.locator("[data-route-counts]")).toHaveText("12 families · 46 exact models · 46 offerings");
  ...
  await expect(catalog.locator("[data-catalog-row]")).toHaveCount(69);
  ...
  await expect(catalog.locator('[data-model="gemini-3.6-flash"]')).toContainText("Reasoning");
  await expect(catalog.locator('[data-model="gemini-3.7-flash"]')).toContainText("Reasoning");
...
test("visitors can filter and choose a model family, exact model, and provider offering", async ({ page }) => {
  ...
  await expect(routingTree.locator('[data-route-weight-access][aria-pressed="true"]')).toHaveCount(1);
  await expect(routingTree.locator('[data-route-capability][aria-pressed="true"]')).toHaveCount(1);
  await expect(counts).toHaveText("12 families · 46 exact models · 46 offerings");
  await expect(routingTree.locator("[data-route-family]:visible")).toHaveCount(12);
  ...
  await expect(counts).toHaveText("8 families · 31 exact models · 31 offerings");
  await expect(routingTree.locator("[data-route-family]:visible")).toHaveCount(8);
  await openWeightsFilter.click();
  await expect(counts).toHaveText("10 families · 35 exact models · 35 offerings");
  ...
  await expect(counts).toHaveText("1 family · 9 exact models · 9 offerings");
  ...
  await expect(routingTree.locator('[data-route-model-group="gemini"] [data-route-model]')).toHaveCount(9);
  ...
  await expect(counts).toHaveText("19 families · 63 exact models · 64 offerings");
...
test("visitors can disclose filters, search every characteristic, and sort through table headers", async ({ page }) => {
  ...
  await expect(resultCount).toHaveText("69 of 69 models");
  ...
  await expect(filterPanel).toBeVisible();
  await expect(resultCount).toHaveText("1 of 69 model");
  ...
  await expect(resultCount).toHaveText("5 of 69 models");
  ...
  await expect(resultCount).toHaveText("9 of 69 models");
  await expect(visibleRows).toHaveCount(9);
  ...
  await expect(resultCount).toHaveText("0 of 69 models");
...
test("reasoning effort is exact to the selected text route and the controls remain responsive", async ({ page }) => {
  ...
  await installAssetRoutes(page);
  await installManagementRoutes(page, { savedProviderIDs: ["openai", "deepseek", "gemini", "meta", "moonshot"] });
  ...
  await expect(textModel).toHaveValue("gemini-2.5-flash");
  ...
  await expect.poll(() => defaultsMutations.some((defaults) => (
    defaults.provider === "gemini" && defaults.model === "gemini-3.7-flash" && defaults.reasoning_effort === "high"
  ))).toBe(true);
