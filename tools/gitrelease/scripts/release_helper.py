...
def version_info(cwd: Path, release_timestamp: dt.datetime) -> dict[str, Any]:
    tags = all_tags(cwd)
    semver_tags = [tag for tag in tags if SEMVER_TAG_RE.fullmatch(tag)]
    latest_semver = semver_tags[0] if semver_tags else None

    return {
        "scheme_guess": "semver",
        "latest_tag": latest_semver,
        "latest_any_version_tag": latest_semver,
        "latest_semver_tag": latest_semver,
        "version_tags": semver_tags[:20],
...
def command_publish_prepared_release(args: argparse.Namespace) -> int:
    ...
    remote_tag_commit = ls_remote_tag_commit(cwd, args.remote, version)
...
def command_publish_release(args: argparse.Namespace) -> int:
    ...
    prerelease = SEMVER_PRERELEASE_TAG_RE.fullmatch(args.version) is not None
    view_command = [
        "gh",
        "release",
        "view",
    ...
    if bool(refreshed.get("isPrerelease")) != prerelease:
        errors.append("GitHub Release object has the wrong prerelease state")
...
def ls_remote_tag_commit(cwd: Path, remote: str, version: str) -> str:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    preflight.add_argument(
        "--release-timestamp",
        help="Release timestamp in ISO format for manifest and changelog metadata.",
    )
    preflight.add_argument(
        "--local",
        action="store_true",
