...
func TestPublicCapabilitiesResponseBoundaries(testingInstance *testing.T) {
	...
	inlineRoute := `"wire_contract":"openai_responses","execution_lifecycle":"pollable_resource","media_execution_lifecycle":"pollable_resource"`
	fileRoute := `"wire_contract":"gemini_interactions","execution_lifecycle":"pollable_resource","media_execution_lifecycle":"synchronous_completion"`
	textRoute := `"wire_contract":"openai_responses","execution_lifecycle":"pollable_resource"`
	...
	testCases := []struct {
		...
		{name: "invalid media limit", response: &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader(`{"offerings":[{"identifier":"provider:model","provider":"provider","model":"model","capabilities":["text"],` + textRoute + `,"media_limits":[{}]}]}`))}},
		{name: "media limit without capability", response: &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader(`{"offerings":[{"identifier":"provider:model","provider":"provider","model":"model","capabilities":["text"],` + textRoute + `,"media_limits":[` + imageCountLimit + `]}]}`))}},
		{name: "audio limit without capability", response: &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader(`{"offerings":[{"identifier":"provider:model","provider":"provider","model":"model","capabilities":["text"],` + textRoute + `,"media_limits":[` + audioCountLimit + `]}]}`))}},
		...
		{name: "media capability without lifecycle", response: &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader(`{"offerings":[{"identifier":"provider:model","provider":"provider","model":"model","capabilities":["image_input","text"],"wire_contract":"openai_responses","execution_lifecycle":"pollable_resource","media_limits":[` + inlineRequestLimit + `,` + imageCountLimit + `,` + imageInlineLimit + `]}]}`))}},
		{name: "invalid media lifecycle", response: &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader(`{"offerings":[{"identifier":"provider:model","provider":"provider","model":"model","capabilities":["image_input","text"],"wire_contract":"openai_responses","execution_lifecycle":"pollable_resource","media_execution_lifecycle":"future","media_limits":[` + inlineRequestLimit + `,` + imageCountLimit + `,` + imageInlineLimit + `]}]}`))}},
		{name: "media lifecycle without capability", response: &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader(`{"offerings":[{"identifier":"provider:model","provider":"provider","model":"model","capabilities":["text"],"wire_contract":"openai_responses","execution_lifecycle":"pollable_resource","media_execution_lifecycle":"pollable_resource","media_limits":[]}]}`))}},
