...
- [x] [I415] (P1) Add an importable Python llm-proxy client package.
  ...
  Resolution: Added `python/llm_proxy_client` with validated `ClientConfig` and `ClientRequest` dataclasses, typed HTTP/transport exceptions, and a transport-only `Client.post` that preserves non-body query fields, strips body-owned fields, and sends the JSON POST contract without model alias rewriting. Added `python/pyproject.toml`, `python/uv.lock`, local-server pytest coverage, README import docs, and Makefile Python lint/test integration. Validation passed with `make python-lint`, `make python-test`, editable install smoke via `uv pip install -e .`, exact local `go install github.com/tyemirov/llm-proxy/llm-proxy-client`, and final `timeout -k 350s -s SIGKILL 350s make ci` with Go total coverage 100.0% and Python pytest 11 passed.
