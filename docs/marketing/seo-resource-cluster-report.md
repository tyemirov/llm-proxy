...
| docs/implementation/provider-routing-plan.md | Implementation notes | Provider routing, config ownership, management, error contract, and adapter notes | Multi-provider routing, model catalogs, omitted-model behavior, split-origin management, usage storage, and provider-key security are implemented contracts. | High |
...
| 21 | Encrypted provider key storage for managed tenants | Teams evaluating how LLM Proxy stores account-owned provider credentials. | Provider API keys are high-value secrets. The management database must store encrypted provider credentials. | encrypted provider key storage | Security | /resources/encrypted-provider-key-storage/ | Low | Maintain existing resource |
...
| 34 | Gemini Interactions proxy for stored 3.x model calls | Developers adding Gemini as a provider without bringing Gemini-specific payloads into every app. | Stored Gemini Interactions require polling and cleanup, which shared proxy callers must not have to coordinate. | Gemini Interactions proxy | Provider routing | /resources/gemini-interactions-proxy/ | Low | Maintain existing resource |
