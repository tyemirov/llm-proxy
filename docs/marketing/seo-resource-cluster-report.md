...
| 21 | Encrypted provider key storage for managed tenants | Teams evaluating how LLM Proxy stores tenant-owned provider credentials. | Provider API keys are high-value secrets. A management database should not store raw upstream credentials as plaintext rows. | encrypted provider key storage | Security | /resources/encrypted-provider-key-storage/ | Low | Maintain existing resource |
