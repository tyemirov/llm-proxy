...
than rerunning CI. `make deploy` passes no repository selector, gateway source
path, sibling path, image override, TAuth argument, or Caddy argument. It runs
only this repository's tracked Ansible transaction and ignored operator
inventory through pinned `ansible-core`. Before Pages branch mutation, that
transaction validates the archive, its source marker, and the remote release
tag. After activation it matches GitHub Pages build state to the pushed branch
commit, then verifies a cache-distinct public marker after backend rollout.
...
The backend consumes TAuth's published Go `pkg/sessionvalidator` for cookie/JWT validation and adds only llm-proxy's tenant, required-expiry, and principal invariants; no application-owned JWT parser or claims schema exists. The deployment manifest declares the tenant and route as data. Repository-owned Ansible stages the selected service inputs and reconciles shared TAuth and Caddy runtime resources before Pages activation; application runtime code contains no TAuth or Caddy deployment orchestration.
