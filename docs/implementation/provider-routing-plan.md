...
than rerunning CI. `make deploy` passes no target, gateway path, image, TAuth, or
Caddy arguments; it invokes the installed neutral `mprlab-deploy` controller,
which reads only this exact repository's declarative resource manifest and
operator inventory. Before Pages branch mutation, the platform controller
...
The backend consumes TAuth's published Go `pkg/sessionvalidator` for cookie/JWT validation and adds only llm-proxy's tenant, required-expiry, and principal invariants; no application-owned JWT parser or claims schema exists. The deployment manifest declares the tenant and route as data. The platform-owned generic controller stages the selected service inputs and reconciles shared TAuth and Caddy resources before Pages activation; application deployment code contains no TAuth or Caddy orchestration.
