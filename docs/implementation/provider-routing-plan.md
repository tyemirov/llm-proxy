...
The masked field accepts the replacement without a separate replacement action.
...
Schema version 12 makes the resolved typed route the only source of usage
provider and model dimensions. A request that fails before route resolution
keeps its totals, status, outcome, latency, and failure data with both route
dimensions blank. A request that fails after route resolution keeps the exact
canonical provider and model. The bounded upgrade clears both dimensions when
an existing pair is not an exact current catalog route for its endpoint. It
preserves every other usage field, verifies the result in the same transaction,
and records version 12. Current startup rejects later dimension drift.
...
The selector is the only tenant name in the card. A tenant change loads only
the open card and preserves the management site context.
The official key link stays beside the API service label. The icon-only key
deletion action stays beside the masked key field. The card autosaves model and
prompt changes. It collapses the prompt when the card or tenant context changes.
The card has no manual completion action.
