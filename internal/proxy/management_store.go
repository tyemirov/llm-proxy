...
type managedTenantDatabase interface {
	...
	connectionCreation(context.Context, string, string) (managedConnectionCreationRecord, error)
	createAccountConnection(context.Context, managedAccountConnectionRecord, managedConnectionCreationRecord) (managedConnectionCreationRecord, error)
	...
	deleteTenant(ownerUserID string, tenantID string, now time.Time) error
...
func (providerKeyCipher managedProviderKeyCipher) decryptConnection(record managedProviderConnectionRecord) (string, error) {
	return providerKeyCipher.decryptConnectionValue(record.TenantID, record.ProviderID, record.FieldID, record.Value)
}

func (providerKeyCipher managedProviderKeyCipher) decryptConnectionValue(connectionID, providerID, fieldID, rawValue string) (string, error) {
	encryptedValue := strings.TrimSpace(rawValue)
	if encryptedValue == constants.EmptyString || !strings.HasPrefix(encryptedValue, managedProviderKeyCiphertextPrefix) {
		return constants.EmptyString, fmt.Errorf("%w: provider=%s field=%s", errManagedProviderKeyDecryption, providerID, fieldID)
	}
	...
	value, decryptError := providerKeyCipher.aeadCipher.Open(nil, nonce, ciphertext, managedProviderConnectionAssociatedData(connectionID, providerID, fieldID))
	if decryptError != nil {
		return constants.EmptyString, fmt.Errorf("%w: provider=%s field=%s: %v", errManagedProviderKeyDecryption, providerID, fieldID, decryptError)
	}
...
func initializeManagedTenantSchema(database *gorm.DB, providerKeyCipher managedProviderKeyCipher, providers *providerRegistry) error {
	return database.Transaction(func(transaction *gorm.DB) error {
		...
		if version.Version > managedTenantSchemaVersion {
			return fmt.Errorf("%w: operation=validate_version version=%d", errManagedTenantSchemaMigration, version.Version)
		}
		if version.Version < managedAccountConnectionsSchemaVersion {
			if err := initializeManagedTenantSchemaRecords(transaction.Session(&gorm.Session{DisableNestedTransaction: true}), providerKeyCipher, providers); err != nil {
				return err
			}
			if err := migrateAccountConnections(transaction, providerKeyCipher, providers); err != nil {
				return err
			}
		}
		if err := validateAccountConnectionSchema(transaction, providerKeyCipher, providers); err != nil {
			return err
		}
...
func initializeManagedTenantSchemaRecords(database *gorm.DB, providerKeyCipher managedProviderKeyCipher, providers *providerRegistry) error {
	...
	if migration.Version >= managedProviderConnectionsSchemaVersion && migration.Version <= managedOpenAITranscriptionSchemaVersion {
	...
	if migration.Version >= managedProviderConnectionsSchemaVersion && migration.Version < managedOpenAITranscriptionSchemaVersion {
		for _, schemaVersion := range managedModelSelectionSchemaVersions {
			...
			if controlError := validateManagedModelMigrationControls(database, managedProviderProfileTable, migrations); controlError != nil {
				return controlError
			}
			for _, modelMigration := range migrations {
				if migrationError := migrateManagedModelSelectionRecords(database, managedProviderProfileTable, modelMigration); migrationError != nil {
					return migrationError
				}
			}
			if recordError := database.Create(&managedSchemaMigrationRecord{Version: schemaVersion, AppliedAt: time.Now().UTC()}).Error; recordError != nil {
				return fmt.Errorf("%w: operation=record_version version=%d: %v", errManagedTenantSchemaMigration, schemaVersion, recordError)
			}
		}
	}
	...
	default:
		return fmt.Errorf("%w: operation=validate_version version=%d expected=%d", errManagedTenantSchemaMigration, migration.Version, managedOpenAITranscriptionSchemaVersion)
	}
...
func migrateManagedModelSelectionsAfter(database *gorm.DB, providerKeyCipher managedProviderKeyCipher, providers *providerRegistry, completedVersion int) error {
	...
	for _, schemaVersion := range managedModelSelectionSchemaVersions {
		if schemaVersion <= completedVersion || schemaVersion >= managedDeepSeekRetirementSchemaVersion {
			continue
		}
...
func migrateManagedModelSelections(database *gorm.DB, providerKeyCipher managedProviderKeyCipher, providers *providerRegistry, schemaVersions ...int) error {
	...
	return database.Transaction(func(transaction *gorm.DB) error {
		for _, migrations := range migrationsByVersion {
			for _, migration := range migrations {
				if migrationError := migrateManagedModelSelectionRecords(transaction, managedProviderProfileTable, migration); migrationError != nil {
					return migrationError
				}
			}
		}
...
func migrateManagedPredecessorModelSelections(database *gorm.DB, providers *providerRegistry, schemaVersion int) error {
	...
	return database.Transaction(func(transaction *gorm.DB) error {
		for _, migration := range migrations {
			if migrationError := migrateManagedModelSelectionRecords(transaction, managedProviderKeyTable, migration); migrationError != nil {
				return migrationError
			}
		}
		return nil
...
// validateManagedModelMigrationControls rejects a profile whose inherited effort
...
func validateManagedModelMigrationControls(database *gorm.DB, profileTable string, migrations []managedModelMigration) error {
	for _, migration := range migrations {
		...
		query := database.Table(managedTenantTable+" AS tenants").Select("tenants.*").
			...
			Where("profiles.provider_id = ? AND profiles.text_model = ?", migration.provider, migration.source).Find(&tenants)
		if query.Error != nil {
			return fmt.Errorf("%w: operation=preflight_reasoning table=%s provider=%s: %v", errManagedTenantSchemaMigration, profileTable, migration.provider, query.Error)
		}
		for _, tenant := range tenants {
			effort := tenant.DefaultReasoningEffort
			for _, defaultMigration := range migrations {
				if tenant.DefaultProvider == defaultMigration.provider && tenant.DefaultModel == defaultMigration.source {
					effort = defaultMigration.targetReasoningEffort
				}
			}
			if effort != migration.targetReasoningEffort && !(effort == "" && migration.targetReasoningEffort == "high") {
				return fmt.Errorf("%w: operation=preflight_reasoning tenant=%s provider=%s source_model=%s reason=provider_reasoning_decision_required", errManagedTenantSchemaMigration, tenant.TenantID, migration.provider, migration.source)
			}
		}
	}
	return nil
...
func migrateManagedKeyedRoutingDefaults(database *gorm.DB, providerKeyCipher managedProviderKeyCipher, providers *providerRegistry) error {
	...
	for _, record := range records {
		providerSettings, providerSettingsError := managedProviderSettingsFromPredecessorRecords(providerKeyCipher, providers, record.ProviderAPIKeys)
		...
		projectedDefaults, _ := canonicalManagedPredecessorDefaults(providers, record.defaults())
...
func preflightManagedQwenCloudRetirement(database *gorm.DB, providerKeyCipher managedProviderKeyCipher, providers *providerRegistry) (managedQwenCloudRetirementDataset, error) {
	retiredModel, migrationError := managedRetiredModelIdentifier(providers, managedQwenCloudRetirementVersion, retiredQwenCloudProviderIdentifier, ModelOperationText)
	if migrationError != nil {
		return managedQwenCloudRetirementDataset{}, migrationError
	}
	records, queryError := managedTenantRecordsForRoutingValidation(database)
	if queryError != nil {
	...
	for _, record := range records {
		...
		providerSettings, providerSettingsError := managedProviderSettingsFromPredecessorRecords(providerKeyCipher, providers, remainingProviderKeys)
		if providerSettingsError != nil {
		...
		currentDefaults, defaultsError := validateManagedQwenCloudRetirementDefaults(providers, retiredModel, record.defaults())
		if defaultsError != nil {
...
func managedRetiredModelIdentifier(providers *providerRegistry, schemaVersion int, provider string, operation string) (string, error) {
	migrations := providers.modelMigrationsFor(schemaVersion, provider, operation)
	if len(migrations) != 1 || migrations[0].target != constants.EmptyString {
		return constants.EmptyString, fmt.Errorf("%w: operation=read_model_migrations version=%d provider=%s operation_kind=%s count=%d", errManagedTenantSchemaMigration, schemaVersion, provider, operation, len(migrations))
	}
	return migrations[0].source, nil
}

func validateManagedQwenCloudRetirementDefaults(providers *providerRegistry, retiredModel string, rawDefaults TenantDefaults) (managedRoutingDefaults, error) {
	if strings.TrimSpace(rawDefaults.Provider) != retiredQwenCloudProviderIdentifier {
		projectedDefaults, _ := canonicalManagedPredecessorDefaults(providers, rawDefaults)
		return validateCanonicalManagedRoutingDefaults(providers, projectedDefaults)
	}
	if rawDefaults.Provider != retiredQwenCloudProviderIdentifier || strings.TrimSpace(rawDefaults.Model) != retiredModel || rawDefaults.Model != retiredModel {
	...
	projectedDefaults, _ := canonicalManagedPredecessorDefaults(providers, currentWithoutRetiredTextRoute)
	...
	validatedDefaults.Model = retiredModel
...
func migrateManagedModelIdentity(database *gorm.DB, providerKeyCipher managedProviderKeyCipher, providers *providerRegistry) error {
	...
	return database.Transaction(func(transaction *gorm.DB) error {
		...
		migratedHistoricalUsage, usageError := managedModelIdentityHistoricalUsage(transaction, providers)
		if usageError != nil {
			return usageError
		}
...
func preflightManagedModelIdentity(database *gorm.DB, providerKeyCipher managedProviderKeyCipher, providers *providerRegistry) (managedModelIdentityDataset, error) {
	...
	for recordIndex := range records {
		...
		for providerKeyIndex := range record.ProviderAPIKeys {
			...
			canonicalModel, changed := canonicalManagedTextModel(providers, providerKey.ProviderID, providerKey.TextModel)
			if changed {
		...
		modelIdentityDefaults, defaultsChanged := canonicalManagedTenantDefaults(providers, record.defaults())
		projectedDefaults, _ := canonicalManagedPredecessorDefaults(providers, modelIdentityDefaults)
	...
	historicalUsage, usageError := managedModelIdentityHistoricalUsage(database, providers)
	if usageError != nil {
...
func canonicalManagedTextModel(providers *providerRegistry, provider string, model string) (string, bool) {
...
func canonicalManagedPredecessorDefaults(providers *providerRegistry, defaults TenantDefaults) (TenantDefaults, bool) {
	modelIdentityDefaults, modelIdentityChanged := canonicalManagedTenantDefaults(providers, defaults)
...
func managedModelIdentityHistoricalUsage(database *gorm.DB, providers *providerRegistry) ([]managedUsageEventRecord, error) {
	migrations := providers.modelMigrations[managedModelIdentitySchemaVersion]
	if len(migrations) == 0 {
		return nil, fmt.Errorf("%w: operation=read_model_migrations version=%d", errManagedTenantSchemaMigration, managedModelIdentitySchemaVersion)
	}
	var historicalUsage []managedUsageEventRecord
	query := database.Where("provider_id = ? AND model_id = ?", migrations[0].provider, migrations[0].source)
	for _, migration := range migrations[1:] {
		query = query.Or("provider_id = ? AND model_id = ?", migration.provider, migration.source)
	}
	queryError := query.
		Order("id").
		Find(&historicalUsage).
...
func validateManagedPendingRouteIdentities(database *gorm.DB, providerKeyCipher managedProviderKeyCipher, providers *providerRegistry) error {
	return validateManagedRoutingDefaults(
		...
		providerKeyCipher,
		providers,
		func(cipher managedProviderKeyCipher, records []managedProviderAPIKeyRecord) (map[providerID]managedProviderSettings, error) {
			return managedProviderSettingsFromPredecessorRecords(cipher, providers, records)
		},
		func(defaults TenantDefaults) TenantDefaults {
			projectedDefaults, _ := canonicalManagedPredecessorDefaults(providers, defaults)
...
func (database *gormManagedTenantDatabase) userByID(userID string) (managedUserRecord, error) {
	...
	queryError := database.database.
		...
		Preload("Tenants.ConnectionAssignments.Connection.Fields").
...
func (database *gormManagedTenantDatabase) deleteTenant(ownerUserID string, tenantID string, now time.Time) error {
	return database.database.Transaction(func(transaction *gorm.DB) error {
		...
		if updateError := transaction.Model(&managedAccountConnectionRecord{}).Where("id IN (?)", assignments).
			Updates(map[string]any{"version": gorm.Expr("version + 1"), "updated_at": now}).Error; updateError != nil {
			return updateError
		}
		if deleteError := transaction.Where(&managedTenantConnectionRecord{TenantID: tenantID}).Delete(&managedTenantConnectionRecord{}).Error; deleteError != nil {
			return deleteError
		}
...
func usageDetailsByTenantIDsInTransaction(transaction *gorm.DB, tenantIDs []string, query managedUsageDetailRecordQuery) ([]managedUsageDetailRecord, uint, error) {
	...
	for _, record := range usageRecords {
		outcomeCode, outcomeError := newManagedUsageOutcomeCode(string(record.OutcomeCode))
		if outcomeError != nil {
			return nil, 0, fmt.Errorf("%w: table=%s id=%d: %w", errManagedTenantStorePersist, managedUsageEventTable, record.ID, outcomeError)
		}
...
func (store *managedTenantStore) updateDefaults(principal managementPrincipal, tenantIdentifier managedTenantIdentifier, defaults managedRoutingDefaults) (managedTenantSnapshot, error) {
	...
	providerSettings, settingsError := store.assignedProviderSettings(record)
	if settingsError != nil {
		return managedTenantSnapshot{}, settingsError
	}
	if _, validationError := validatePersistedManagedRoutingDefaults(store.routingDefaults, providerSettings, defaults.value()); validationError != nil {
		return managedTenantSnapshot{}, fmt.Errorf("%w: tenant=%s: %w", errManagementDefaults, tenantIdentifier.string(), validationError)
	}
...
func (store *managedTenantStore) snapshot(record managedTenantRecord) (managedTenantSnapshot, error) {
	providerSettings, providerKeyError := store.assignedProviderSettings(record)
...
func managedProviderSettingsFromPredecessorRecords(providerKeyCipher managedProviderKeyCipher, providers *providerRegistry, providerKeyRecords []managedProviderAPIKeyRecord) (map[providerID]managedProviderSettings, error) {
	...
	for predecessorProviderIdentifier, settings := range predecessorSettings {
		...
		textModel, _ := canonicalManagedTextModel(providers, providerIdentifier, settings.textModel)
