...
func TestModelActivationMigratesStoredSelectionsAtStartup(t *testing.T) {
	for _, hasMigration := range []bool{true, false} {
		...
		t.Run(name, func(t *testing.T) {
			...
			router, err := buildRouterForTest(t, withInternalUpstreamCapacity(t, Configuration{ProviderCatalog: catalog, Management: management, AssetStorePath: t.TempDir()}), zap.NewNop().Sugar())
			if !hasMigration {
