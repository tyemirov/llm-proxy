...
func newManagedResolvedUsageRouteMigrationFixture(t *testing.T) (*gorm.DB, managedProviderKeyCipher, *providerRegistry, []managedUsageEventRecord) {
	...
	records := []managedUsageEventRecord{
		...
		{ID: 3, TenantID: tenant.TenantID, Endpoint: usageEndpointDictation, ProviderID: ProviderNameOpenAI, ModelID: DefaultTranscriptionModel, StatusCode: http.StatusOK, Disposition: managedUsageDispositionSucceeded, OutcomeCode: managedUsageOutcomeSuccess, LatencyMilliseconds: 23, CreatedAt: now.Add(2 * time.Minute)},
