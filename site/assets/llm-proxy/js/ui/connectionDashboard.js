...
import {MODEL_TASKS, tasksForOffering, offeringMatchesTasks, reconcileSelectedTasks, toggleSelectedTask, renderTaskFlow, renderTaskDetails, renderTaskPicker} from '../modelTasks.js?v=20260903f037';
...
export class ConnectionDashboard extends HTMLElement {
  ...
  adaptCapabilityForConnection() {
    ...
    this.taskIDs = reconcileSelectedTasks(this.taskIDs, this.availableTasks);
  }
  ...
  renderMap() {
    ...
    const modelIDs = this.attached && this.ready ? this.modelsForSelectedTasks(this.taskIDs) : [];
    map.innerHTML = `<svg class="cw-wires" aria-hidden="true"></svg>
      ...
      <section class="cw-column"><header><h3>Models</h3></header>${renderTaskPicker(this.availableTasks,this.taskIDs,this.busy)}<div class="cw-list">
      ${modelIDs.filter(matches).map(id=>{const saved=this.isDefaultModel(id);const offering=this.offerings.find(o=>o.provider===this.connection?.provider && o.model===id); const selectedOfferingTasks=offering ? tasksForOffering(offering).filter(t=>this.taskIDs.includes(t.id)) : [];return `<button class="cw-node ${saved?'selected':this.modelID===id?'preview':''}" data-model="${escapeHTML(id)}" ${disabled}><span class="cw-row"><brand-icon kind="family" identifier="${escapeHTML(this.modelFamilies[id])}"></brand-icon><code>${escapeHTML(id)}</code></span>${selectedOfferingTasks.map(task=>renderTaskFlow(task)).join('')}${saved?'<small class="cw-connected">★ Default model</small>':''}</button>`;}).join('') || `<p class="cw-empty">${this.attached ? this.ready ? (this.provider?.capabilities.length===0 && this.provider.resources.length ? 'This connection provides account resources.' : 'No models for the selected tasks.') : 'Add credentials to choose models. Edit this connection to complete its setup.' : 'Connect to choose models.<br>Use a Connect button in the middle column.'}</p>`}</div></section>`;
  ...
  handleClick(event) {
    ...
    if (taskID && this.availableTasks.some(value=>value.id===taskID)) {
      ...
      if (taskButton instanceof HTMLElement) taskButton.focus();
      return;
    }
